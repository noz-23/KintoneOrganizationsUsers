...
(async (PLUGIN_ID_) => {
  ...
  /* 
  ユーザー選択へのデータ入れ
   引数　：events_ イベントデータ, readField_ 読み取りフィールド名, apiFieldUsers_ 実行するAPI
   戻り値：なし
  */
  const setUsers = async (events_, readField_, apiFieldUsers_) => {
    console.log('setUsers:%o %o', readField_, apiFieldUsers_);

    let listSetUser = [...events_.record[writeUsers].value];
    console.log('listSetUser:%o', listSetUser);

    // 組織(グループ)情報の取得
    for (let field of events_.record[readField_].value) {
      // 組織(グループ)に所属する人の取得
    ...
    if (editUsers != 'true') {
      if (events_.record[readField_].value.length == 0) {
        listSetUser = [];
      }
    }
